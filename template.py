...
def generate_gpt(data,name,basic_info,major_preferences,college_preferences,potential_major_exploration,column_AU):
    ...
    gpt_input = f"{major_prompt_one},{data}"
    chat_completion = openai.chat.completions.create(
        model="gpt-4",
        messages=[{"role": "user", "content": gpt_input}],
        temperature=0.5,
...
def main():
    ...
    try:
        result = subprocess.run(['xelatex', template_path], check=True, capture_output=True, text=True)
        print("PDF generated successfully.")
    except subprocess.CalledProcessError as e:
        print(f"Failed to generate PDF: {e.stderr}")
