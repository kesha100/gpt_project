...
def process_csv_and_generate_content(df, name):
    try:
...
def generate_gpt(data,name,basic_info,major_preferences,college_preferences,potential_major_exploration,column_AU):
    ...
    major_prompt_three = f"""请根据以下补充信息（被3个引号括起）继续从上述10个专业中进行第二步的3个最匹配专业的筛选。请给出更详细的理由，每个理由都需要要足够多的细节，证据和推理过程。每个理由都不能少于300字。请同时给出专业匹配度（0为最不匹配，100为最匹配）：'''{college_preferences}'''"""
    ...
    #     你现在模仿的是大学中的顾问
    #     请根据上述8个维度分别给出以下三个专业的缺点：'''{major_list}'''。每个专业的每个维度都需要列出，如果在这个维度上没有明显缺点，请直接说明。
    ...
    # Correspondence_college_recommendations = f"""学生希望未来到'''{column_AU}'''国家读大学本科，可能专业为'''{major_list}'''；请你为他推荐这些国家中每个国家里三个专业分别的具有代表性的两所大学并分别给出详细的推荐理由，每个推荐理由不少于300字。"""
    ...
    # Major_development_history = f"""请列出以下三个专业过去50年历史中的每个专业5个最重要的转折点及其时间与影响：'''{major_list}'''。每个转折点及其影响的描述不能少于200字"""

    # Cutting_edge_field = f"""请列出以下三个专业每个专业学术界的3个最前沿领域和工业界的3个最前沿领域：'''{major_list}'''。请详细描述每个领域，每个领域的描述均不能少于200字。"""
    ...
    Visualization_p1 = f"""1. 知识掌握程度（Knowledge Mastery）：这个维度可以通过测试或者评估来测量学生对于该学科的核心概念和技能的理解程度。这可能包括学生的课堂表现、作业、项目、测试和考试成绩。 
        ...
        请根据以下问卷结果对该学生的’工程学科', '理科学科', '社会科学学科', '社会人文学科‘分别对相应的五个维度进行打分（0-5分）： use this related information'''{potential_major_exploration}''' """
    
    # Visualization_p2 = f"""请参考以下括号内段落的格式与内容和问卷信息，为用户的推荐专业'''{major_list}'''以及它们所对应的五大维度‘知识掌握程度’，‘热爱程度’，‘实践实用能力‘，‘创新能力‘，‘对未来的投入意愿‘，写一个分析并为每个专业的每个维度进行打分(0-5分)。 
    # (运动医学： 
    ...
    # 问卷信息'''{basic_info}、{major_preferences}、{college_preferences}'''。"""
...
# 请给出有创造性,独特性的活动规划。每一个活动的细节与信息不能少于300字。"""
...
def main():
    ...
    final_content = template_content.replace("%generated_Visualization_p1_%", generated_Visualization_p1)
    ...
    try:
        ...
        print("PDF generation command output:", result.stdout.decode())
